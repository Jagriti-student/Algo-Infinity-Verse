// ===== QUIZ DATA =====
window.roadmapTabsInitialized = window.roadmapTabsInitialized || false;
window.initRoadmap = window.initRoadmap || function () {};
window.initGamification = window.initGamification || function () {};
const quizQuestions = {
  arrays: [
    {
      id: "arrays-1",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // DAILY CHALLENGE
  if (typeof initDailyChallenge === "function") {
    initDailyChallenge();
  }
...
function initializeAnimations() {
  console.log("✔ initializeAnimations FIXED (temp)");
}
function renderTopics() {
  console.log("✔ renderTopics FIXED (temp)");
}
// ===== LOADING SCREEN =====
...
function initLoadingScreen() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      loader.remove(); // final cleanup (important)
    }, 400);
...
function selectQuizAnswer(selectedIndex) {
  ...
  // Highlight selection
  const quizOptionsEl = document.getElementById('topicQuizOptions');
  quizOptionsEl.querySelectorAll('.quiz-option').forEach((opt, idx) => {
    opt.classList.add('selected');
    if (idx === question.correct) {
      opt.classList.add('correct');
    } else if (idx === selectedIndex && !isCorrect) {
...
function renderProblems(filter = "all", searchQuery = "") {
  ...
  data-id="${problem.id}" aria-label="Favorite problem">
          <i class="fas fa-heart"></i>
                 ...
                 <button class="notes-btn ${userProgress.problemNotes[problem.id] ? "has-notes" : ""
  ...
  window.saveProblemNotes = saveProblemNotes;
...
if (!window.practiceProblems) {
  ...
  function initRoadmap() {
    ...
    if (progressBar && stages.length >= 3) {
      const totalProblems = practiceProblems?.length || 1;
      const completed = userProgress?.completedProblems?.length || 0;
  ...
  function updateDashboard() {
    ...
    window.renderPersonalityCard = window.renderPersonalityCard || function () {
    console.log("personality card fallback");
};
    ...
    window.renderMistakeDnaCard = window.renderMistakeDnaCard || function () {
    console.log("mistake DNA fallback");
};
  }
...
function solveProblem() {
  ...
  console.log("LOAD USER DATA FILE REACHED");
  window.loadUserData = function () {
    ...
    initProfile();
} else if (typeof window.initProfile === "function") {
    window.initProfile();
}
      });
  }
  ...
  window.closeTopicModal = function closeTopicModal() {
    const topicModal = document.getElementById("topicModal");
  ...
  function closeQuizEditor() {
    ...
    if (quizEditorModal) {
        quizEditorModal.classList.remove("active");
    }
...
if (runBtn) {
  runBtn.addEventListener("click", runPerl);
}
...
print "Welcome $name\\n";`;
  ...
  if (profileDate) profileDate.textContent = formattedDate;
}
  // optional: auto refresh every hour (safe for daily date change)
  setInterval(() => {
...
function loadUserData() {
    ...
    // Example UI updates (if elements exist)
...
window.initChatbot = window.initChatbot || function () {
...
window.initDailyChallenge = window.initDailyChallenge || function () {
    console.log("Daily challenge initialized");
};
function killOverlays() {
  const ids = ["loading-screen"];
  const classes = [".nav-overlay", ".chatbot-widget"];
...
setTimeout(() => {
  ...
  document.querySelectorAll(".nav-overlay, .chatbot-widget").forEach(el => el.remove());

  document.body.style.overflow = "auto";

  console.log("🔥 ALL OVERLAYS KILLED");
}, 0);
const topics = document.querySelector("#topics");
if (topics) {
